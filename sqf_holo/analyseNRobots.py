...
algorithms = ['../target_holonomic_topdown_influence']
...
def plotTheoreticalMeanLimits(nRobots,a,my_marker,my_label,axes=None):
  ...
  lowerLim, upperLim = [fhmin(radius_s,upperD[i],lowerV[i]) for i in range(len(nRobots))], [fhmin(radius_s,lowerD[i],upperV[i]) for i in range(len(nRobots))]
...
def plotMeanLimitsFromHistogram(nRobots,a,my_marker,my_label,axes):
  ...
  radius_s = 3
  sufx = "_d"
  counts = [loadFile("bin/countsSQF-"+sufx+"-"+str(nRobots[n])+"-"+holoSubStr+".bin") for n in range(len(nRobots))]
  bins = [np.array(loadFile("bin/binsSQF-"+sufx+"-"+str(nRobots[n])+"-"+holoSubStr+".bin")) for n in range(len(nRobots))]
...
def plotTheoreticalLimits(nRobots,a,my_marker,axes):
  maxVelocityLine = 12
  minDistanceLine = 11
  radius_s = 3
  ...
  limits = [fhmin(radius_s, minds[n], maxvs[n]) for n in range(len(nRobots))]
  # ~ limits2 = [fhmin(radius_s, min(minds), max(maxvs))]*len(nRobots)
  axes.plot(nRobots,limits,color='tab:orange',marker=my_marker)
  # ~ axes.plot(nRobots,limits2,'g--')

def plotTheoreticalInstantaneous(nRobots,a,my_marker,axes):
  maxVelocityLine = 12
  minDistanceLine = 11
  D = 13
  radius_s = 3
  maxvs = [max(data[n, a, :nSamples[a], maxVelocityLine]) for n in range(len(nRobots))]
  minds = [min(data[n, a, :nSamples[a], minDistanceLine]) for n in range(len(nRobots))]
  if os.path.isfile('bin/ths.bin'):
    ths = loadFile('bin/ths.bin')
  else: 
    ths = [bestTheta((D - radius_s)/maxvs[n], radius_s, minds[n], maxvs[n], 1000)[1] for n in range(len(nRobots))]
    saveFile('bin/ths.bin',ths)
  limits = [throughput((D - radius_s)/maxvs[n], radius_s, minds[n], maxvs[n], ths[n]) for n in range(len(nRobots))]
  axes.plot(nRobots,limits,color='tab:orange',marker=my_marker)
...
def mainLoop(fileline):
  ...
  for a in range(len(algorithms)):
    if fileline != 19:
      ...
      if plotMaxLimit:
        ...
        plotMeanLimitsFromHistogram(nRobots,a,algorithmsSymbol[a],"Mean values (hist)",ax2)
        plotTheoreticalLimits(nRobots,a,algorithmsSymbol[a],ax1)
        ax1.set_title("Least bound of hex. pack. lim.")
        for ax in fig.get_axes():
          ax.label_outer()
      else:
        plotTheoreticalMeanLimits(nRobots,a,algorithmsSymbol[a],"Least bound of hex. pack. lim.")
        plt.errorbar(nRobots,dataMean[:,a,fileline], yerr=[m - n for m,n in zip(dataUpCi[:,a,fileline],dataMean[:,a,fileline])],label=algorithmsLabels[a],marker=algorithmsSymbol[a],capsize=5);
  ...
  if fileline != 19:
    ...
    plt.savefig("SQF-throughput-th-exp-"+holoSubStr+".pdf",bbox_inches="tight",pad_inches=0.01);
    plt.savefig("SQF-throughput-th-exp-"+holoSubStr+".png",bbox_inches="tight",pad_inches=0.01);
  plt.clf();
