...
for j in `seq 0 $((NUMTESTS-1))`; 
  do
  echo "*** Experimenting the time increasing in relation to the radius: log number $j. ****"
  rad=`echo "$radStart" | awk '{printf "%.1f", $1}'`
  while [ `echo "$rad $radEnd" | awk '{printf "%i", $1 <= $2}'` != 0  ]; 
  do
    if [ ! -e rad$rad/log\_$j ]; then
      logfile=$(generateRandomFile . log)
      echo "******* test.sh $auto $nRobots 0.3 $logfile $rad ********" &>> $analysisLogFile
      ./test.sh $auto $nRobots 0.3 $logfile $rad &>> $analysisLogFile
      if [ ! -d rad$rad ]; then
        mkdir rad$rad;
      fi
      ...
      if [ $i -le $((NUMTESTS-1)) ]; then
        ...
        #After it is created, if the log file is empty, the experiment is done again at most MAXTRIES times
        try=0;
